<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:65:"D:\Wamp\www\shop\public/../application/admin\view\order\list.html";i:1594011635;s:58:"D:\Wamp\www\shop\application\admin\view\common\navbar.html";i:1593923731;s:59:"D:\Wamp\www\shop\application\admin\view\common\sidebar.html";i:1593924689;}*/ ?>
<!DOCTYPE html>
<!--
...
<link rel="shortcut icon" href="/shop/public/static/assets/img/favicon.png"
	type="image/x-icon">
...
<!--Beyond styles-->
<link id="beyond-link" href="/shop/public/static/assets/css/beyond.min.css"
	rel="stylesheet" type="text/css" />
...
<!-- /Page Sidebar -->
			...
			<div class="page-content">
				...
				<div class="dashboard-box" style="margin: 16px; padding: 16px">
					<ul class="nav nav-tabs tabs-flat" id="tabList" style="margin-bottom: 12px;">
                               <li <?php if($cond['isSend'] == '0'): ?>class="active"<?php endif; ?> onClick="changeStatus(0)">
                                   <a data-toggle="tab" href="#home11" aria-expanded="true">
                                       未发货
                                   </a>
                              </li>
                              <li <?php if($cond['isSend'] == '1'): ?>class="active"<?php endif; ?> onClick="changeStatus(1)">
					...
					<div id="editabledatatable_filter" class="dataTables_filter">
						<form id="orderForm">
							<input type="hidden" name="isSend" value="<?php echo $cond['isSend']; ?>">
							
							<label><input name="orderId" value="" type="search" value="<?php echo $cond['orderId']; ?>"
								class="form-control input-sm" placeholder="根据订单号匹配"
								aria-controls="editabledatatable"></label> 
							<a onclick="$('#orderForm').submit()"
								class="btn btn-default" style="height: 30px; margin-left: 10px;">搜索</a>
						</form>
					</div>
						...
						<thead>
							<tr role="row">
								<th class="sorting_asc" tabindex="0"
									...
									aria-label="
                                     Username
                                 : activate to sort column descending">订单号</th>
								<th class="sorting" tabindex="0"
									aria-controls="editabledatatable" rowspan="1" colspan="1"
									aria-label="
                                     Full Name
                                 : activate to sort column ascending">下单时间</th>
								<th class="sorting" tabindex="0"
									aria-controls="editabledatatable" rowspan="1" colspan="1"
									aria-label="
                                     Points
                                 : activate to sort column ascending">会员名<br>&手机号</th>
								<th class="sorting" tabindex="0"
									style="width:10%;"
									aria-controls="editabledatatable" rowspan="1" colspan="1"
									aria-label="
                                     Points
                                 : activate to sort column ascending">备注</th>
								<th class="sorting" tabindex="0"
									aria-controls="editabledatatable" rowspan="1" colspan="1"
									aria-label="
                                     Points
                                 : activate to sort column ascending">支付方式<br>&实付</th>
								<th class="sorting" tabindex="0"
									aria-controls="editabledatatable" rowspan="1" colspan="1"
									aria-label="
                                     Points
                                 : activate to sort column ascending">状态</th>
								<th class="sorting" tabindex="0"
									aria-controls="editabledatatable" rowspan="1" colspan="1"
									aria-label="
                                     Points
                                 : activate to sort column ascending">收货人姓名<br>&手机号</th>
								<th class="sorting" tabindex="0"
									style="width:15%;"
									aria-controls="editabledatatable" rowspan="1" colspan="1"
									aria-label="
                                     Points
                                 : activate to sort column ascending">收货地址</th>
								<th class="sorting" tabindex="0"
									style="width:15%;"
									aria-controls="editabledatatable" rowspan="1" colspan="1"
									aria-label="
                                     Points
                                 : activate to sort column ascending">快递单号</th>
						</thead>

						<tbody>
							<tr role="row">
								<td><?php echo $order[0]['id']; ?></td>
								<td><?php echo $order[0]['createTime']; ?></td>
								<td><?php echo $order[0]['memberName']; ?><br><?php echo $order[0]['phone']; ?></td>
								<td><?php echo $order[0]['postscript']; ?></td>
								<td>
									<?php switch($order[0]['payMethod']): case "0": ?>支付宝支付<?php break; case "1": ?>微信支付<?php break; default: ?>网上银行：<br>￥<?php echo $order[0]['totalPay']; endswitch; ?>
								</td>
								<td>
									<?php switch($order[0]['status']): case "0": ?>未付款<?php break; case "1": ?>已付款，未发货<?php break; case "2": ?>已发货，未确认收货<?php break; case "3": ?>已确认收货，未评价<?php break; default: ?>已完成
									<?php endswitch; ?>
								</td>
								<td><?php echo $order[0]['consigneeName']; ?><br><?php echo $order[0]['consigneePhone']; ?></td>
								<td><?php echo $order[0]['province']; ?> <?php echo $order[0]['city']; ?> <?php echo $order[0]['area']; ?><br><?php echo $order[0]['detail']; ?></td>
								<td>
									<div class="input-group input-group-sm">
										<?php switch($order[0]['status']): case "1": ?>
											<input type="text" class="form-control" placeholder="快递单号" id="deliveryId_<?php echo $order[0]['id']; ?>">
											<span class="input-group-btn">
                                                 <a onClick="sendBefore(this, 'deliveryId_<?php echo $order[0]['id']; ?>')" 
                                                 href="<?php echo url('order/send'); ?>?orderId=<?php echo $order[0]['id']; ?>" class="btn btn-default" >发货</a>
                                             </span>
										<?php break; case "0": ?>
											<input type="text" class="form-control" value="买家未付款" disabled>
										<?php break; default: ?>
											<input type="text" class="form-control" value="<?php echo $order[0]['deliveryId']; ?>" disabled>
										<?php endswitch; ?>
							...
							<tr class="details">
								<td class="details" colspan="9" style="padding-top:16px;">
									<div class="row">
											...
											<div class="row">
												...
												<div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
													<div class="databox bg-white radius-bordered">
														<div class="databox-left bg-themesecondary" style="padding:0px;">
															<a href="<?php echo url('index/goods/index'); ?>?id=<?php echo $item['goodsId']; ?>" target="blank">
															<img style="width:65px;height:65px;" src="/shop/public/<?php echo $item['mainPic']; ?>"/>
															</a>
														</div>
														<div class="databox-right">
															<span class="databox-number" style="font-size:14px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;">
																<?php echo $item['goodsName']; ?>
															</span>
															<div style="font-size:16px;" class="databox-text">￥<?php echo $item['salePrice']; ?> × <b class="themesecondary"><?php echo $item['count']; ?></b></div>
														</div>
													</div>
												</div>
												<?php endforeach; ?>
	...
	<script>
		...
		function sendBefore(obj, id) {
			var deliveryId = $("#" + id).val();
			console.log(deliveryId);
			if (confirm('请确认快递单号：' + deliveryId + '，是否确认发货？')) {
				var newHref = $(obj).attr('href') + '&deliveryId=' + deliveryId;
		...
		function changeStatus(isSend) {
			$("input[name='isSend']").val(isSend);
			$("#orderForm").submit();
		}
